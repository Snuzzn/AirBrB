...
import { AiOutlineUserAdd, AiOutlineLogin, AiOutlineHome, AiOutlineLogout } from 'react-icons/ai'
import { HiUserCircle } from 'react-icons/hi'
import { Link } from 'react-router-dom';

export default function Navbar () {
  const userOptions = [{ icon: <AiOutlineUserAdd alt="Register icon: user being added"/>, text: 'Register', to: '/register', loggedIn: false }, { icon: <AiOutlineLogin alt="Login icon: arrow entering circle"/>, text: 'Login', to: '/login', loggedIn: false }, { icon: <AiOutlineHome alt="Your listings Icon: house icon"/>, text: 'Your Listings', to: '/hosted-listings', loggedIn: true }, { icon: <AiOutlineLogout alt='Logout icon: arrow leaving circle' />, text: 'Logout', loggedIn: true, onClick: () => { localStorage.removeItem('token') } }]
